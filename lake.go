...
func (c *Client) WriteRFC6902(ctx context.Context, catlog string, reqJSON []byte) (*WriteResult, error) {
	...
	*/
	...
	tsSeq, err := c.writer.GetTimeSeqID(ctx, catlog)
	if err != nil {
		return nil, fmt.Errorf("failed to generate time+seqid: %w", err)
	}
	...
	key := storage.MakeDataKey(catlog, tsSeq, int(index.MergeTypeRFC6902))
	if err := c.storage.Put(ctx, key, reqJSON); err != nil {
	...
	err = c.writer.AddWithTimeSeq(ctx, tsSeq, catlog, "RFC6902", index.MergeTypeRFC6902)
	if err != nil {
		return nil, fmt.Errorf("failed to add to index: %w", err)
	}
	return &WriteResult{
...
func (c *Client) applyPatchWithAutoCreate(merged []byte, patchData []byte) ([]byte, error) {
	...
	result, err := decoder.Apply(merged)
	if err != nil {
		return nil, err
	}
...
func (c *Client) mergeEntries(ctx context.Context, catalog string, baseData []byte, entries []index.DataInfo) ([]byte, error) {
	...
	for _, entry := range entries {
		...
		case index.MergeTypeRFC6902:
			// Apply patch with auto-creation of missing parent paths
			merged, err = c.applyPatchWithAutoCreate(merged, data)
			if err != nil {
